...
interface Minter:
    def mint(_gauge: address, _for: address): nonpayable
...
def lzReceive(_lz_chain_id: uint16, _lz_address: Bytes[40], _nonce: uint64, _payload: Bytes[64]):
    ...
    if issued > self.limit or self.is_killed:
        ...
        self.integrate_fraction[receiver] += amount

        Minter(MINTER).mint(self, receiver)
...
def retry(_nonce: uint64, _timestamp: uint256, _receiver: address, _amount: uint256):
    ...
    self.integrate_fraction[_receiver] += _amount

    Minter(MINTER).mint(self, _receiver)
