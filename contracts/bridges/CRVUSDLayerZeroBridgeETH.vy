...
ALT_CRVUSD: public(immutable(address))
...
@external
def __init__(_delay: uint256, _limit: uint256, _lz_chain_id: uint16, _alt_crvusd: address):
    self.delay = _delay
    log SetDelay(_delay)
...
def bridge(
    ...
    assert _amount != 0 and _receiver not in [empty(address), ALT_CRVUSD]  # dev: invalid
...
def retry(_nonce: uint64, _timestamp: uint256, _receiver: address, _amount: uint256):
    ...
    ERC20(CRVUSD).transfer(_receiver, _amount)
